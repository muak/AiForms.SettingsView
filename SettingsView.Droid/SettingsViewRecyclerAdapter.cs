...
namespace AiForms.Renderers.Droid
{
	...
	{
		float _MinRowHeight => _context.ToPixels(44);
		...
		List<ViewHolder> _ViewHolders { get; set; } = new List<ViewHolder>();
		...
		void SettingsView_ModelChanged(object sender, EventArgs e)
		{
		...
		void OnSectionPropertyChanged(object sender, System.ComponentModel.PropertyChangedEventArgs e)
		{
			...
			{
				UpdateSectionTextAllignment(section);
			}
		}

		void UpdateSectionVisible(Section section)
		{
			var indexes = _proxy.Select((x, idx) => new { idx, x.Section })
		...
		void UpdateSectionTextAllignment(Section section)
		{
			var index = _proxy.FindLastIndex(x => x.Section == section);
			NotifyItemChanged(index);

			foreach ( var element in _ViewHolders )
			{
				if ( element is HeaderViewHolder header )
				{
					header.TextView.TextAlignment = CellBaseView.GetTextAllignment(section.TextAlignment);
				}

				if ( element is FooterViewHolder footer )
				{
					footer.TextView.TextAlignment = CellBaseView.GetTextAllignment(section.TextAlignment);
				}
			}
		}
		...
		public void OnClick(AView view)
		{
			...
			if ( !( view.FindViewById<LinearLayout>(Resource.Id.ContentCellBody).GetChildAt(0) is CellBaseView cell ) || 
				!_proxy[position].Cell.IsEnabled )
			{
		...
		public virtual bool OnLongClick(AView v)
		{
			...
			if ( !( v.FindViewById<LinearLayout>(Resource.Id.ContentCellBody).GetChildAt(0) is CellBaseView cell ) || 
				!_proxy[position].Cell.IsEnabled )
			{
				//if FormsCell IsEnable is false, does nothing. 
				return false;
		...
		void BindHeaderView(HeaderViewHolder holder)
		{
			...
			//update text
			holder.TextView.TextAlignment = CellBaseView.GetTextAllignment(section.TextAlignment);
